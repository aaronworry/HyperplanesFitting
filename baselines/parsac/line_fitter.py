...
PARSAC 原生支持：
- 消失点检测 (vp): 从线段预测消失点
- 基础矩阵估计 (fundamental): 从点对匹配估计 F 矩阵
- 单应矩阵估计 (homography): 从点对匹配估计 H 矩阵

由于 PARSAC 不直接支持 2D 点云的多直线拟合，我们采用以下策略：
...
方案2：直接使用 PARSAC 的框架实现自定义直线拟合
- 这需要修改 PARSAC 的网络和数据加载
- 更复杂但更准确
...
class SimplePARSACLineFitter:
    ...
    def fit(
        ...
        Args:
            points: (N, 2) 点云
            num_models: 模型数量（默认使用 self.num_instances）
            num_iterations: 迭代次数
