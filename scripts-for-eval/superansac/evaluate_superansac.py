...
# -*- coding: utf-8 -*-
"""
SupeRANSAC (Sequential RANSAC) 评估脚本 - 使用与论文完全一致的评估指标

在 csv_dataset 上评估 Sequential RANSAC，输出格式与其他对比方法一致
"""
...
from sequential_ransac import SequentialRANSAC2DLine, RANSACConfig


def run_superansac_on_sample(data: np.ndarray, 
                              num_lines: int = None,
                              max_iterations: int = 1000,
                              inlier_threshold: float = 0.3,
                              min_inliers: int = 10) -> tuple:
    """
    在单个样本上运行 Sequential RANSAC
    ...
    lines, labels = fitter.fit(data, max_models=num_lines or 10, min_points_ratio=0.05)
    runtime = time.time() - start_time
    
    # 转换为 Hyperplane 对象
    # lines 是 (n1, n2, d) 格式的 numpy 数组列表
    hyperplanes = []
    for line in lines:
...
def evaluate_dataset(data_dir: str,
    ...
    for i in range(num_samples):
        ...
        hyperplanes, runtime = run_superansac_on_sample(
            data, 
            num_lines=num_lines,
...
def save_results(results: list, output_dir: str, method_name: str = "superansac"):
    """保存评估结果"""
...
def main():
    parser = argparse.ArgumentParser(description='SupeRANSAC (Sequential RANSAC) 评估脚本')
    ...
    output_dir = args.output_dir or os.path.join(project_root, 'results', 'superansac')
    
    print("=" * 50)
    print("SupeRANSAC (Sequential RANSAC) 评估")
    print("=" * 50)
    ...
    print(f"最小内点数: {args.min_inliers}")
    print("=" * 50)
