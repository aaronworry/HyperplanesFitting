...
class Multisource_Hyperplanes_Locations():
    ...
    def optimal_construction(self):
        ...
        for i in range(self.num_hyperplanes):
            self.model.addConstr(gp.quicksum(self.hyperplanes_normal[i, j]*self.hyperplanes_normal[i, j] for j in range(self.dim)) == 1.)
        ...
        for i in range(self.num_points):
            for j in range(self.num_hyperplanes):
                ...
                self.model.addConstr(self.distance_term[i, j] == self.e[i], name=f"pow_dis_{i}_{j}")
