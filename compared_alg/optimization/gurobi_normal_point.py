...
class Multisource_Hyperplanes_Locations():
    def __init__(self, num, GN, Gp, data, tol=1.e-9):
        self.tol = tol
    ...
    def set_initial_value(self):
        ...
        for key, value in self.hyperplanes_point.items():
            self.hyperplanes_point[key].Start = float(self.ground_truth_p[key])
    ...
    def optimal_construction(self):
        ...
        for i in range(self.num_points):
            for j in range(self.num_hyperplanes):
                distance_expr = gp.quicksum(self.hyperplanes_normal[j, k] * (self.data[i, k] - self.hyperplanes_point[j, k]) for k in range(self.dim))
    ...
    def solve(self):
        ...
        if self.model.status == GRB.OPTIMAL:
            ...
            return self.sol_N, self.sol_p, t_diff
...
if __name__ == "__main__":
    ...
    A[0, 0] = N[0, 0]
    A[0, 1] = N[0, 1]
    b[0, 0] = - N[0, 0]*p[0, 0] - N[0, 1] * p[0, 1]      # N(x-p) = 0
    print(A, b, time)
