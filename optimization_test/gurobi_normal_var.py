...
class Multisource_Hyperplanes_Locations():
    def __init__(self, num, GN, Gd, data, tol=1.e-9):
        ...
        self.var = self.model.addVars(self.num_hyperplanes, vtype=GRB.CONTINUOUS, name="var")
        self.sum_of_squares = self.model.addVars(self.num_hyperplanes, vtype=GRB.CONTINUOUS, name="sum_of_squares")
        self.square_of_sum = self.model.addVars(self.num_hyperplanes, vtype=GRB.CONTINUOUS, name="square_of_sum")
        self.points_of_hyperplanes = self.model.addVars(self.num_hyperplanes, vtype=GRB.INTEGER, lb=0.0, name="points_of_hyperplanes")
        # z_ij
        self.z = self.model.addVars(self.num_points, self.num_hyperplanes, vtype=GRB.BINARY, name="z")
        self.distance_term = self.model.addVars(self.num_points, self.num_hyperplanes, vtype=GRB.CONTINUOUS, name="distance")
    ...
    def optimal_construction(self):
        ...
        for i in range(self.num_points):
            for j in range(self.num_hyperplanes):
                self.model.addConstr(gp.quicksum(self.hyperplanes_normal[j, k] * self.data[i, k] for k in range(self.dim)) == self.distance_term[i, j])
        
        """
        for j in range(self.num_hyperplanes):
            # mean
            self.model.addConstr(self.points_of_hyperplanes[j] == gp.quicksum(self.z[i, j] for i in range(self.num_points)))
            self.model.addConstr(self.hyperplanes_distance[j] == gp.quicksum(self.z[i, j]*self.distance_term[i, j] for i in range(self.num_points)) / self.points_of_hyperplanes[j])
            
            self.model.addConstr(self.sum_of_squares[j] ==  gp.quicksum(self.z[i, j] * self.distance_term[i, j] * self.distance_term[i, j] for i in range(self.num_points)))
            self.model.addConstr(self.var[j] == self.sum_of_squares[j] / self.points_of_hyperplanes[j] - self.hyperplanes_distance[j]**2) 
        """
        
        for j in range(self.num_hyperplanes):
            self.model.addConstr(self.sum_of_squares[j] ==  gp.quicksum(self.z[i, j] * self.distance_term[i, j] * self.distance_term[i, j] for i in range(self.num_points)))
            self.model.addConstr(self.square_of_sum[j] == gp.quicksum(self.z[i, j] * self.distance_term[i, j] for i in range(self.num_points)) ** 2)
            self.model.addConstr(self.points_of_hyperplanes[j] == gp.quicksum(self.z[i, j] for i in range(self.num_points)))
            self.model.addConstr(self.hyperplanes_distance[j] == gp.quicksum(self.z[i, j]*self.distance_term[i, j] for i in range(self.num_points)) / self.points_of_hyperplanes[j])
            # Objective: Minimize variance (1/n) * (Σ(xᵢ²) - (1/n) * (Σ(xᵢ))²)
            self.model.addConstr(self.var[j] == (self.sum_of_squares[j] - self.square_of_sum[j] / self.points_of_hyperplanes[j]) / self.points_of_hyperplanes[j])
        
        
        obj = gp.quicksum(self.var[j] for j in range(self.num_hyperplanes))
